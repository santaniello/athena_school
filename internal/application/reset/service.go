...
// invariant every other knowledge mutation already keeps (see ADR-004).
// It never touches the OpenRouter config or the onboarding profile.
func (s *Service) ResetLocalData(ctx context.Context) error {
	...
	if err := s.vectorStore.ReplaceAll(ctx, nil); err != nil {
		return fmt.Errorf("reset: clearing the vector index: %w", err)
	}
	return nil
}
