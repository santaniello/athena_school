...
func TestFindDuplicates_returnsExactMatch_withoutEmbeddingCall(t *testing.T) {
	...
	items.EXPECT().
		FindByNormalizedConcept(mock.Anything, "System Design", "cache aside pattern").
	...
	matches, err := service.FindDuplicates(context.Background(), candidate, domainknowledge.DefaultDuplicateTopK, domainknowledge.DefaultDuplicateSimilarity)
...
func TestFindDuplicates_embedsConceptAndDefinition_andSearchesTopicScopedAthenaChunks(t *testing.T) {
	...
	items := knowledgemocks.NewMockRepository(t)
	items.EXPECT().FindByNormalizedConcept(mock.Anything, "System Design", "circuit breaker").Return(nil, nil)
	items.EXPECT().GetByID(mock.Anything, "item-1").
	...
	store.EXPECT().
		Search(mock.Anything, []float32{0.1, 0.2, 0.3}, domainknowledge.DefaultDuplicateTopK,
	...
	llm := llmmocks.NewMockProvider(t)
	llm.EXPECT().
		Embeddings(mock.Anything, domainllm.EmbeddingRequest{Input: "Circuit Breaker\n\nA resiliency pattern."}).
...
func TestFindDuplicates_raisingTheThreshold_changesWhichMatchesSurvive(t *testing.T) {
	...
	matches, err := service.FindDuplicates(context.Background(), candidate, domainknowledge.DefaultDuplicateTopK, 0.75)
...
func TestFindDuplicates_includesASemanticMatchExactlyAtTheThreshold(t *testing.T) {
	...
	matches, err := service.FindDuplicates(context.Background(), candidate, domainknowledge.DefaultDuplicateTopK, 0.9375)
...
func TestFindDuplicates_returnsError_whenExactLookupFails(t *testing.T) {
	...
	items := knowledgemocks.NewMockRepository(t)
	items.EXPECT().FindByNormalizedConcept(mock.Anything, "System Design", "circuit breaker").
