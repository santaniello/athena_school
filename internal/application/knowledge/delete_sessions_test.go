...
func newSessionDeleteService(
	...
	return NewService(nil, nil, chunks, tx, store, guard, domainknowledge.RetrievalThresholds{})
}

func TestDeleteSessionsWithKnowledge_collectsChunkIDsBeforeTheDelete_thenEvictsTheIndex(t *testing.T) {
	...
	runWithinTx(tx)
	service := newSessionDeleteService(chunks, tx, store, passingIndexGuard(t))
	logged := captureLog(t)
	...
	// foreign-key cascade, their chunks) disappear, and the index is evicted last
	require.NoError(t, err)
	assert.Equal(t, []string{"ids-a", "ids-b", "delete", "evict"}, order)
	assert.Empty(t, logged.String(), "a clean eviction logs nothing")
}

func TestDeleteSessionsWithKnowledge_returnsTheDeleteError_withoutEvictingTheIndex(t *testing.T) {
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := newSessionDeleteService(chunks, tx, knowledgemocks.NewMockVectorStore(t), passingIndexGuard(t))

	// When deleting
	err := service.DeleteSessionsWithKnowledge(ctx, []string{"session-a"}, func(context.Context) error { return boom })

	// Then the error propagates; index eviction never runs (the mock would
	// fail on any unexpected call), so nothing is half-cleaned after the
	// rollback
...
func TestDeleteSessionsWithKnowledge_returnsTheListingError_withoutRunningTheDelete(t *testing.T) {
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := newSessionDeleteService(chunks, tx, nil, passingIndexGuard(t))
	deleted := false
...
func TestDeleteSessionsWithKnowledge_returnsErrIndexLoading_whenIndexIsLoading_andNeverDeletes(t *testing.T) {
	...
	service := newSessionDeleteService(nil, nil, nil, guard)
	deleted := false
