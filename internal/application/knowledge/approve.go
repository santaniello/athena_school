...
// the gap and write it back over this transition (see Transactor). The
// same transaction updates every owned chunk's status metadata; after
// commit, the VectorStore is reconciled to match without a new embedding
// call. A post-commit reconciliation failure never rolls back the durable
// transition — it comes back as an error wrapping ErrIndexingFailed
// alongside the real, transitioned item.
//
// If id owns no chunk yet (UpdateMetadataByItemID matched zero rows —
// e.g. a prior SaveDrafts indexing attempt failed), the transition still
// commits and indexKnowledgeItem attempts the recoverable embedding
// afterwards, instead of reconciling an empty metadata update.
