...
func TestDeleteItem_cascadesToChunks_thenDeletesTheItem_thenCleansUpUnreferencedEvidence_thenReconcilesTheStore(t *testing.T) {
	...
	runWithinTx(tx)
	service := NewService(repository, nil, chunks, tx, store, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, evidenceRepo)
...
func TestDeleteItem_propagatesChunkDeletionError_withoutDeletingTheItem(t *testing.T) {
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := NewService(repository, nil, chunks, tx, nil, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, nil)
...
func TestDeleteItem_propagatesEvidenceCleanupError_insideTheSameTransaction(t *testing.T) {
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := NewService(repository, nil, chunks, tx, nil, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, evidenceRepo)
