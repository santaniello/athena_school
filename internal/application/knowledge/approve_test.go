...
func TestApprove_transitionsDraftToApproved_andReconcilesChunkMetadata(t *testing.T) {
	...
	runWithinTx(tx)
	service := NewService(repository, nil, chunks, tx, store, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, nil)
...
func TestApprove_returnsInvalidTransition_whenItemIsAlreadyApproved(t *testing.T) {
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := NewService(repository, nil, nil, tx, nil, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, nil)
...
func TestApprove_returnsErrIndexLoading_whenIndexIsLoading_andNeverTouchesTheRepository(t *testing.T) {
	...
	service := NewService(repository, nil, nil, nil, nil, guard, domainknowledge.RetrievalThresholds{}, nil)
...
func TestApprove_reindexes_whenTheItemOwnsNoChunkYet(t *testing.T) {
	// Given a draft item with no existing chunk — e.g. its original
	// SaveDrafts indexing attempt failed, so UpdateMetadataByItemID
	// matches zero rows
	ctx := context.Background()
	repository := knowledgemocks.NewMockRepository(t)
	repository.EXPECT().GetByID(ctx, "item-1").Return(domainknowledge.Item{
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := NewService(repository, llm, chunks, tx, store, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, nil)
...
func TestApprove_returnsErrIndexingFailed_whenRecoveryReindexingFails_butKeepsTheDurableTransition(t *testing.T) {
	// Given a draft item with no existing chunk, and an embedding call that fails
	ctx := context.Background()
	repository := knowledgemocks.NewMockRepository(t)
	repository.EXPECT().GetByID(ctx, "item-1").Return(domainknowledge.Item{
		ID: "item-1", Topic: "Go", Concept: "Channels", Definition: "Typed conduits.",
	...
	tx := txmocks.NewMockTransactor(t)
	runWithinTx(tx)
	service := NewService(repository, llm, chunks, tx, nil, passingIndexGuard(t), domainknowledge.RetrievalThresholds{}, nil)
