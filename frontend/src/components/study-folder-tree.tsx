...
// "Start new session" context-limit action) force its sidebar to reflect a
// session created through a different flow than this tree's own
// handleStartSession.
  ...
  function StudyFolderTree(
    ...
    useImperativeHandle(ref, () => ({
      refreshFolder(folderId: string) {
        setFolders((previous) =>
          previous.map((folder) => (folder.id === folderId ? { ...folder, open: true } : folder)),
        )
